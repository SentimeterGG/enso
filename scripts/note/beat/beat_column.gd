...
@export_category("Judgment")
@export var excellent_window_ms := 30.0
@export var good_window_ms := 60.0
@export var bad_window_ms := 100.0
...
func _try_hit_note(input_time: float) -> void:
	...
	if best != null:
		...
		var judgment := _get_judgment(timing_error)

		best.hit(judgment)


func _get_judgment(timing_error: float) -> String:
	var error_ms := absf(timing_error) * 1000.0

	if error_ms <= excellent_window_ms:
		return "excellent"

	if error_ms <= good_window_ms:
		return "good"

	if error_ms <= bad_window_ms:
		return "bad"
